    ...
    <div className="flex flex-col mt-2">
      <a href={checkoutUrl} target="_self">
        <Button
          as="span"
          width="full"
          className="block bg-dark-blue text-white border-none w-full py-3 text-center mt-5 uppercase"
        >
          Proceed to Checkout
        </Button>
      </a>
...
function ItemRemoveButton({ lineIds }: { lineIds: CartLine['id'][] }) {
  ...
  return (
    <fetcher.Form action="/cart" method="post">
      ...
      <button className="border-none focus-within:outline-none" type="submit">
        <span className="sr-only">Remove</span>
        <IconClose aria-hidden="true" className="w-3 h-3" />
      </button>
    </fetcher.Form>
  );
}
      ...
      <div className="flex items-center border rounded">
        <UpdateCartButton lines={[{ id: lineId, quantity: prevQuantity }]}>
          <button
            ...
            className="w-10 h-10 transition disabled:text-gray-600"
            value={prevQuantity}
            disabled={quantity <= 1}
          >
            <span>&#8722;</span>
