...
export async function loader({ params, request, context }: LoaderArgs) {
  ...
  const contentfulEndpoint = `https://cdn.contentful.com/spaces/${CONTENTFUL_SPACE_ID}/environments/master/entries?access_token=${CONTENTFUL_ACCESS_TOKEN}&content_type=activePromotions&fields.name=mxusa-active-promotions`;

  const promoResponse = await fetch(contentfulEndpoint)
    .then((res) => {
      return res.json();
    });
...
export default function CollectionProducts() {
  ...
  const { collection, promoResponse } = useLoaderData<typeof loader>();
  ...
  const [productType, setProductType] = useState<string>(
    (products[0] as Product).productType,
  );
  ...
  useEffect(() => {
    if (state?.product) {
      const _product = products.find(
        (prod) => prod.handle === (state.product as Product).handle,
      );
      setSelectedProduct(_product!);
    }
    if (queryProductType && selectedProduct.productType != queryProductType) {
      const _product = (products as Product[]).filter(
        (product: Product) => product.productType == queryProductType,
      )[0];
      setSelectedProduct(_product);
      setSelectedImage(_product.featuredImage || _product.images.nodes[0]);
    }
  }, []);
  ...
  const changeProduct = (_product: Product) => {
    ...
    setProductType(_product.productType);
  ...
  return (
    <div className="w-full pb-6 flex flex-wrap justify-between">
      <div className="w-full lg:w-9/12">
        <div className="product-image w-full relative h-[350px] sm:h-[480px] md:h-[500px] lg:h-auto">
          <Swiper
            slidesPerView={1}
            loop
          ...
          <div className="product-menu-bottom absolute right-0 left-0 bottom-0 bg-white z-10">
            {(products as ProductWithMetafields<Product>[]).filter(
              (product) => {
                if (!queryProductType) return true;
                else return product.productType == queryProductType;
              },
            ).length > 1 ? (
              <div className="flex">
                <div
                ...
                {productType == 'Mattress' && (
                  <>
                    <div
                      ...
                      className={`flex grow justify-between items-center p-2 md:px-4 md:py-5 break-all text-174860 text-10 md:text-xs font-semibold uppercase border border-border cursor-pointer ${handle === 'technology' && 'bg-174860 text-white'
                        }`}
                    >
                ...
                {productType == 'Mattress' && (
                  <>
                    <div
                      ...
                      className={`flex grow justify-between items-center p-2 md:px-4 md:py-5 break-all text-174860 text-10 md:text-xs font-semibold uppercase border border-border cursor-pointer ${handle === 'comparison' && 'bg-174860 text-white'
                        }`}
                    >
                ...
                <AnchorLink
                  href="#productDetails"
                  className={`hidden md:flex grow justify-between items-center p-2 md:px-4 md:py-5 break-all text-174860 text-10 md:text-xs font-semibold uppercase border border-border cursor-pointer`}
                  offset={100}
                >
              ...
              <div className="w-full flex justify-center border border-border cursor-pointer">
                <AnchorLink
                  href="#productDetails"
                  className={`hidden md:flex grow justify-between items-center p-2 md:px-4 md:py-5 break-all text-174860 text-10 md:text-xs font-semibold uppercase`}
                  offset={100}
                >
                  <span>Discover more</span>
                  <svg
            ...
            )}
            {
              ...
              //   queryProductType={queryProductType || undefined}
              //   productType={productType}
              //   handle={handle}
              //   selectedProduct={selectedProduct}
              //   changeProduct={changeProduct}
                ...
                {(products as Product[])
                  .filter((product: Product) => {
                    if (!queryProductType) return true;
                      ...
                      return product.productType == queryProductType;
                    }
                  })
